...
### Fixed
- `datacontract test` against SQL Server / Microsoft Fabric with `DATACONTRACT_SQLSERVER_AUTHENTICATION=cli` no longer fails with `Invalid value specified for connection string attribute 'Authentication'`, and `ActiveDirectoryInteractive` reports that it is Windows-only instead of timing out (#1603)
