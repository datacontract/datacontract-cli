...
### Added
- Resolve `authoritativeDefinitions` of `type: definition` on schema properties: the referenced definition document is fetched and its `logicalType`, `businessName`, `description`, `examples`, `classification`, `tags`, type constraints, `enum`, `pii`, `precision`, and `scale` fill in any fields the property leaves unset. A reference may be a relative path (resolved against `ENTROPY_DATA_HOST`), an absolute URL, an IRI, or a local file; the `x-api-key` header is sent only to the configured entropy-data host (self-hosted or SaaS), never to a third-party URL a contract names. Inline values always win, resolution is in-memory only (the contract file is never changed), and an unresolvable reference is skipped with a warning — an `authoritativeDefinitions` link is informational in ODCS, so a broken one never fails the contract. This restores the definition inlining that became a no-op in 0.11.1 when the internal model moved to ODCS.
