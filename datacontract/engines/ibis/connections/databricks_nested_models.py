...
def _nested_model_name(parent_model: str, field_path: str) -> str:
    return f"{parent_model}__{field_path.replace('.', '__')}"


def _nested_field_expr(field_path: str) -> str:
...
def _collect_databricks_virtual_model_specs(
    ...
    for prop in properties or []:
        ...
        if prop_type == "array" and prop.items and prop.items.properties:
            virtual_model = _nested_model_name(parent_model, field_path)
            specs[virtual_model] = {
                "parent": parent_model,
                "field": _nested_field_expr(field_path),
                "template": "SELECT __dc_nested__.* FROM {source} LATERAL VIEW OUTER explode_outer({field}) AS __dc_nested__",
            }
            _collect_databricks_virtual_model_specs(virtual_model, prop.items.properties, specs)
