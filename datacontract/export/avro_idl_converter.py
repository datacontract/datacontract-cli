...
class AvroIDLExporter(Exporter):
    def export(self, export_args) -> dict:
        ...
        return to_avro_idl(self.dict_args.get("data_contract"))
...
def generate_model_types( contract: DataContractSpecification) -> list[AvroModelType]:
    ...
    for model_name, model in contract.models.items():
        result.append(
            AvroModelType(
                name=model_name, description=model.description, fields=_generate_field_types(contract)
            )
        )
    return result
...
def _contract_to_avro_idl_ir( contract: DataContractSpecification) -> AvroIDLProtocol:
    ...
    protocol_name = (
        _model_name_to_identifier(contract.info.title) if contract.info and contract.info.title else None
    )
