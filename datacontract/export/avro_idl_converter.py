            ...
            match field.type:
                case "array":
                    return AvroArrayField(
                        field_name, field.required, field.description, self._to_avro_idl_type(field_name, field.items)
                    )
                case "object" | "record" | "struct":
                    return AvroComplexField(
                        ...
                        field.required,
                        field.description,
                        [self._to_avro_idl_type(field_name, field) for (field_name, field) in field.fields.items()],
                    )
                case _:
                    raise DataContractException(
                        type="general",
        ...
        match field:
            ...
            case AvroComplexField(name, required, _, subfields):
                ...
                for subfield in subfields:
                    subfield_types.append(self._write_field_type_definition(subfield, indent + 1, stream))
                # Reference all defined record types.
                for field, subfield_type in zip(field.subfields, subfield_types):
                    self._write_field_description(field, indent + 1, stream)
                    self._write_indent(indent + 1, stream)
            ...
            case AvroArrayField(name, required, _, item_type):
                subfield_type = self._write_field_type_definition(item_type, indent, stream)
                if required is True:
