...
class DBMLExporter(Exporter):
    ...
    def to_dbml_diagram(self, contract: spec.DataContractSpecification, server: spec.Server) -> str:
        ...
        result += self.generate_project_info(contract) + "\n"

        for model_name, model in contract.models.items():
            table_description = self.generate_table(model_name, model, server)
            result += f"\n{table_description}\n"
    ...
    def add_generated_info(self, contract: spec.DataContractSpecification, server: spec.Server) -> str:
        ...
        datacontract_cli_version = self.get_version()
    ...
    */
    ...
    Note: "{1}"
        ...
        for field_name, field in model.fields.items():
            ref, field_string = self.generate_field(field_name, field, model_name, server)
            if ref is not None:
