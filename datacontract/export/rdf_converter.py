...
class RDFExporter(Exporter):
    ...
    def to_rdf(self, data_contract_spec: DataContractSpecification, base) -> Graph:
        ...
        self.add_info(contract=this_contract, info=data_contract_spec.info, graph=g, dc=dc, dcx=dcx)

        if data_contract_spec.terms is not None:
            self.add_terms(contract=this_contract, terms=data_contract_spec.terms, graph=g, dc=dc, dcx=dcx)

        for server_name, server in data_contract_spec.servers.items():
            self.add_server(contract=this_contract, server=server, server_name=server_name, graph=g, dc=dc, dcx=dcx)

        for model_name, model in data_contract_spec.models.items():
            self.add_model(contract=this_contract, model=model, model_name=model_name, graph=g, dc=dc, dcx=dcx)

        for example in data_contract_spec.examples:
            self.add_example(contract=this_contract, example=example, graph=g, dc=dc, dcx=dcx)
    ...
    def add_triple(self, sub, pred, obj, graph, dc, dcx):
        if pred == "ref":
            ...
            for item in getattr(obj, pred):
                self.add_predicate(sub=sub, pred=pred, obj=item, graph=graph, dc=dc, dcx=dcx)
