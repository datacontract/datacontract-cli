...
# Provenance tag marking a test the CLI created from nothing (safe to delete).
GENERATED_TAG = "dc:generated"
# Marker on a column the CLI created solely to host its tests (dropped when it becomes testless).
COLUMN_MARKER = "datacontract_cli"
...
def _test_tags(entry: str | dict) -> list:
    tags = (_test_body(entry).get("config") or {}).get("tags")
    return list(tags) if isinstance(tags, list) else []


def _is_managed_test(entry: str | dict) -> bool:
    return OUTPUT_TAG in _test_tags(entry)


def _is_generated_test(entry: str | dict) -> bool:
    return GENERATED_TAG in _test_tags(entry)


def _test_dc_type(entry: str | dict) -> Optional[str]:
    """The classification suffix from the `dc:<type>` tag, if exists."""
    for t in _test_tags(entry):
        if t.startswith("dc:") and t != GENERATED_TAG:
            return t[len("dc:") :]
    return None


def _ensure_config_tags(body: dict) -> list:
    """Return the (created if missing) `config.tags` list of a test body, ready to mutate."""
...
def _ensure_membership_tags(entry: dict, dc_type: Optional[str], *, generated: bool) -> None:
    """Add `datacontract_cli` (+ `dc:<type>`, + `dc:generated` if we created it) to a test's tags."""
    name = _test_name(entry)
    body = entry[name]
    tags = _ensure_config_tags(body)
    wanted = [OUTPUT_TAG]
    if dc_type:
        wanted.append(f"dc:{dc_type}")
    if generated:
        wanted.append(GENERATED_TAG)
    for t in wanted:
        if t not in tags:
            tags.append(t)


def _strip_managed_tags(entry: dict) -> bool:
    """Remove the CLI's tags (`datacontract_cli`, `dc:*`) from a test, leaving the user's test.
    ...
    if not isinstance(tags, list):
        return False
    kept = [t for t in tags if t != OUTPUT_TAG and not (isinstance(t, str) and t.startswith("dc:"))]
...
def _collapse_test(entry: str | dict) -> str | dict:
    """Collapse `{name: {}}` back to the bare string `name` (e.g. after un-tagging an adopted test)."""
    name = _test_name(entry)
...
def _reconcile_test(existing: str | dict, desired: dict, dc_type: Optional[str]) -> dict:
    ...
    for k, v in desired_args.items():
        body[k] = v
    _ensure_membership_tags(entry, dc_type, generated=False)
    return entry
...
def _reconcile_data_tests(container: dict, desired_tests: list) -> None:
    ...
    for desired in desired_tests:
        ...
        if idx is None:
            ...
            _ensure_membership_tags(new_entry, dc_type, generated=True)
            existing.append(new_entry)
        else:
            existing[idx] = _reconcile_test(existing[idx], desired, dc_type)
...
def _is_managed_column(col: dict) -> bool:
    meta = col.get("meta")
    return isinstance(meta, dict) and meta.get(COLUMN_MARKER) is True
...
def _entry_has_footprint(entry: dict) -> bool:
    if _meta_data_contract(entry):
        return True
    if any(_is_managed_test(t) for t in (entry.get("data_tests") or [])):
        return True
    for col in entry.get("columns") or []:
        if isinstance(col, dict) and (
            _is_managed_column(col) or any(_is_managed_test(t) for t in (col.get("data_tests") or []))
        ):
            return True
    return False
    ...
    *,
    ...
    for entry in tests:
        ...
        if model_required and (column_key, name) in required_keys:
            ...
            continue
        if _is_generated_test(entry):
            continue  # drop the node we created
        _strip_managed_tags(entry)  # un-tag adopted test, keep the user's
