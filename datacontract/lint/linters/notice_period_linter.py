...
class NoticePeriodLinter(Linter):
    ...
    def lint_implementation(self, contract: DataContractSpecification) -> LinterResult:
        ...
        if not period.startswith("P"):
            return LinterResult.erroneous(f"Notice period '{period}' is not a validISO8601 duration.")
        if period == "P":
            return LinterResult.erroneous(
                "Notice period 'P' is not a validISO8601 duration, requires at least oneduration to be specified."
            )
        if (
