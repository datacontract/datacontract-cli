...
def resolve_data_contract_from_location(
    ...
    return _resolve_data_contract_from_str(
        data_contract_str, schema_location, inline_references, all_errors, base_location=location
    )
    ...
    for schema_obj in data_contract.schema_:
        if schema_obj.properties:
            for prop in schema_obj.properties:
                inline_definition_into_property(prop, base_location, visited)
...
def inline_definition_into_property(
    ...
    if prop.items is not None:
        inline_definition_into_property(prop.items, base_location, visited)
    if prop.properties is not None:
        for nested_prop in prop.properties:
            inline_definition_into_property(nested_prop, base_location, visited)
    ...
    if _is_local_reference(url):
        definition = _resolve_local_definition(url, base_location, visited)
    else:
...
def _resolve_local_definition(url: str, base_location: str | None, visited: frozenset[str]) -> SchemaProperty:
    ...
    if fragment:
        contract = _load_local_contract(url, target_path, visited)
        definition = _lookup_fragment(url, contract, fragment)
    else:
        definition = _load_local_property(url, target_path, visited)
...
def _load_local_contract(url: str, contract_path: Path, visited: frozenset[str]) -> OpenDataContractStandard:
    ...
    try:
        contract = _resolve_data_contract_from_str(read_resource(key))
    except DataContractException as e:
        raise _local_resolution_error(url, f"'{key}' is not a valid data contract: {e.reason}", original_exception=e)

    inline_definitions_into_data_contract(contract, base_location=key, visited=visited | {key})
    _local_contract_cache[key] = contract
    return contract


def _load_local_property(url: str, property_path: Path, visited: frozenset[str]) -> SchemaProperty:
    ...
    try:
        document = _to_yaml(read_resource(key))
    except DataContractException as e:
    ...
    inline_definition_into_property(definition, base_location=key, visited=visited | {key})
    return definition
...
def _local_resolution_error(url: str, detail: str, original_exception: Exception | None = None):
    ...
    if is_open_data_contract_standard(yaml_dict):
        ...
        if inline_references:
            inline_definitions_into_data_contract(
                odcs, base_location=base_location, visited=_initial_visited(base_location)
            )
        return odcs
