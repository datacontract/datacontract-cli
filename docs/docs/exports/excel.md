...
A custom property that carries only a `property` and a scalar `value` is written **inline**: every row sheet, the schema sheets and the `Servers` sheet have a group of columns headed *Custom Properties (add as needed)*, and each column is named after one property, with the value in the row's cell. The template provides three unnamed columns; the export names them as it goes and adds more when a sheet needs them (on `Servers`, where servers are columns, the properties are rows). A custom property with an `id`, `description` or `vendor`, or whose value would not survive a plain cell — `null`, a list, an object, or a string such as `"true"`, `"007"` or `"3.10"` that Excel would read back as something else — goes to the `Custom Properties` sheet instead, with a `Type` of `Text` (verbatim string) or `JSON` (array or object). The contract's, the description's, the team's and a schema's own custom properties always live on that sheet, except `owner`, which fills the `Owner` cell on `Fundamentals`.
