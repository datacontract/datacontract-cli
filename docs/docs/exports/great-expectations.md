...
- `checks` — comma-separated list of check categories to export: `properties` (constraints inferred from logical types) and/or `quality` (rules from the contract's `quality` blocks). Omit to export both, matching the previous behavior.
