...
Custom properties are read both from the inline `Custom Property` / `Custom Value` pairs next to an element and from the `Custom Properties` sheet, and merged. When the same property appears in both places, the sheet wins and a warning is logged.
