...
Authentication options and data type handling for [Azure storage connections](../connect/azure.md).
...
The service principal needs the **Storage Blob Data Reader** role. The [metadata-check path](../connect/azure.md#metadata-checks) alternatively accepts `DATACONTRACT_AZURE_CONNECTION_STRING` or `DATACONTRACT_AZURE_STORAGE_ACCOUNT_KEY`.
...
`physicalType` is never checked against file sources. Schema objects with `logicalType: blob` / `physicalType: file` switch to [metadata checks](../connect/azure.md#metadata-checks) against blob properties instead of reading file contents.
