...
func Open(dataContractFileName string, dataContractStudioUrl string) error {
	contractUrl, err := createDataContractInStudio(dataContractFileName, dataContractStudioUrl)
	if err != nil {
		return err
	}
...
func createDataContractInStudio(dataContractFileName string, dataContractStudioUrl string) (string, error) {
	file, err := os.ReadFile(dataContractFileName)
	if err != nil {
		return "", err
	}

	id, err := newDataContract(dataContractStudioUrl)
	if err != nil {
		return "", err
	}
...
func newDataContract(dataContractStudioUrl string) (string, error) {
	r, err := postForm(dataContractStudioUrl+"/new", nil)
	if err != nil {
		return "", fmt.Errorf("failed to create new data contract id: %w", err)
	}

	strings := strings.Split(r.Request.URL.Path, "/")

	if len(strings) < 2 {
		return "", fmt.Errorf("cant extract data contract id from %v", r.Request.URL.Path)
	}
...
func sendDataContract(dataContractStudioUrl string, id string, file []byte) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to send data contract: %w", err)
	}
