...
def test_json_optional_field_present_in_new_data():
    """Optional field present in new JSON data should pass validation"""
    data_contract = DataContract(
        data_contract_file="fixtures/schema-evolution/odcs-datacontract-cities-version-2.yaml", server="current-json"
    )
...
def test_data_from_historical_and_current_schema_json_mixed():
    """Combining JSON data written with historical and current schema passes validation"""
    data_contract = DataContract(
        data_contract_file="fixtures/schema-evolution/odcs-datacontract-cities-version-2.yaml",
        server="mixed-schema-json",
    )
