...
def _example(logical_type: str) -> SchemaProperty:
    """A property of the given logical type; a vector needs its dimensions to have a native type."""
    options = {"dimensions": 1536} if logical_type == "vector" else None
    return SchemaProperty(name="example", logicalType=logical_type, logicalTypeOptions=options)


def sql_table(server_type: str, column: str) -> str:
    rows = [f"| `{lt}` | {_convert(_example(lt), convert_to_sql_type, server_type)} |" for lt in LOGICAL_TYPES]
